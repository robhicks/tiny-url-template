...
Query.prototype.set = function set () {
    ...
    while ( len-- ) { q[ len ] = arguments$1[ len ]; }
 ...
 */
TinyUri.prototype.authority = function authority (authority$1) {
    if ( authority$1 === void 0 ) { authority$1 = ''; }

  if (authority$1 !== '') {
    var auth = authority$1.match(this.authRegEx);
    this._authority = authority$1;
    if (auth) {
      authority$1 = authority$1.replace(this.authRegEx, '');
      this.userInfo(auth[1]);
    }
    var port = authority$1.match(this.portRegEx);
    if(port) {
      authority$1 = authority$1.replace(this.portRegEx, '');
      this.port(port[1]);
    }
    this.host(authority$1.replace('{', ''));
    return this;
  } else {
    var userinfo = this.userInfo();
    if (userinfo) { authority$1 = userinfo + '@'; }
    authority$1 += this.host();
    var port$1 = this.port();
    if (port$1) { authority$1 += ':' + port$1; }
    return authority$1;
  }
};
